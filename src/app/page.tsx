...
export default function Home() {
  ...
  const myDraftPositions = useMemo(() => {
    const real = (myRoster?.players ?? [])
      .map((id) => playerPosition(players[id]))
      .filter(Boolean) as string[];
    const practiceMine = practicePicks
      .filter((p) => p.mine)
      .map((p) => playerPosition(players[p.playerId]))
      .filter(Boolean) as string[];
  ...
  const recommendations = useMemo<Recommendation[]>(() => {
    ...
    return Object.values(players)
      ...
      .map((player) => {
        ...
        return { player, rating, seasonProjection: projection };
      })
      .filter((x): x is Recommendation => Boolean(x.rating))
  ...
  const lineupMoves = useMemo(() => {
    ...
    myRoster.starters.forEach((starterId, index) => {
      ...
      const decision = lineupDecision(
        { player: starter, projection: weeklyProjection(starterId) },
        eligible[0],
      );
  ...
  async function selectLeague(selected: SleeperLeague, preserveTab = false) {
    ...
    try {
      ...
      const intel = intelResponse.ok
        ? await intelResponse.json() as IntelligencePayload
        : { weekly: {}, season: {}, weekNumber: week, source: "unavailable" };
  ...
  function mockPick(playerId: string, mine = true) {
    setPracticePicks((old) => [...old, { playerId, mine }]);
  }

  function simulateOtherManagers() {
    const amount = Math.max(1, (league?.total_rosters ?? 10) - 1);
    const available = recommendations.slice(0, amount);
    setPracticePicks((old) => [...old, ...available.map((r) => ({ playerId: r.player.player_id, mine: false }))]);
  }

  if (!league) {
    return <main className="shell">
      <section className="card setup">
        <div className="eyebrow">NFL FANTASY WITHOUT THE NFL HOMEWORK</div>
        <div className="brand"><h1>🏈 Fantasy Copilot</h1><p className="muted">Connect Sleeper once. The app turns your league into simple decisions.</p></div>
  ...
  return <main className="shell">
    <header className="topbar">
      <div className="brand"><h1>🏈 Fantasy Copilot</h1><div className="muted"><span className="leagueName">{league.name}</span> · Week {currentWeek} · {formatScoring(scoring)}</div></div>
      <div className="row"><button className="btn secondary" onClick={() => selectLeague(league, true)} disabled={loading}>{loading ? "Refreshing…" : "↻ Refresh"}</button><button className="btn secondary" onClick={() => setLeague(null)}>Change league</button></div>
    </header>
    ...
    {error && <p className="error">{error}</p>}
    {lastUpdated && <div className="syncLine"><span className="statusDot" /> Synced with Sleeper {lastUpdated.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} · Projection feed: {Object.keys(intelligence.weekly).length ? "available" : "limited"}</div>}
...
function Dashboard({ league, roster, activeDraft, players, recommendations, waiverTargets, lineupMoves, matchups, rosters, currentWeek, weeklyProjection, setTab, practiceMode }: any) {
  ...
  return <>
    <section className="decisionHero card">
      <div><div className="eyebrow">WHAT SHOULD I DO?</div><h2>{lineupMoves[0] ? `Make ${lineupMoves.length} lineup change${lineupMoves.length > 1 ? "s" : ""}` : injuries.length ? `Check ${injuries.length} injury flag${injuries.length > 1 ? "s" : ""}` : practiceMode ? "Practice your draft" : "You are mostly set"}</h2><p className="muted">{lineupMoves[0] ? `${nameOf(lineupMoves[0].in)} projects ${lineupMoves[0].delta} points above ${nameOf(lineupMoves[0].out)}.` : injuries[0] ? `${nameOf(injuries[0])} is currently listed ${injuries[0].injury_status}.` : practiceMode ? "Build a mock roster and let the dashboard react to every pick." : "No obvious high-priority move is showing right now."}</p></div>
      <button className="btn" onClick={() => setTab(lineupMoves[0] || injuries.length ? "lineup" : practiceMode ? "draft" : "waivers")}>Open recommendation →</button>
    </section>
    ...
    <div className="grid">
      ...
      <section className="card span8"><div className="sectionHeader"><div><div className="eyebrow">ACTION QUEUE</div><h2 className="sectionTitle">Things worth your attention</h2></div></div><div className="actions">
        {lineupMoves.slice(0, 2).map((move: any) => <button className="action good clickable" key={`${move.in.player_id}-${move.out.player_id}`} onClick={() => setTab("lineup")}><strong>Start {nameOf(move.in)} over {nameOf(move.out)}</strong><div className="muted">+{move.delta} projected points · {move.confidence} confidence</div></button>)}
        {injuries.slice(0, 2).map((p: SleeperPlayer) => <button className="action warn clickable" key={p.player_id} onClick={() => setTab("lineup")}><strong>Monitor {nameOf(p)}</strong><div className="muted">{p.injury_status}{p.injury_body_part ? ` · ${p.injury_body_part}` : ""}{p.practice_participation ? ` · ${p.practice_participation}` : ""}</div></button>)}
        {practiceMode && top && <button className="action clickable" onClick={() => setTab("draft")}><strong>Practice draft recommendation: {nameOf(top.player)}</strong><div className="muted">Current decision score {top.rating.score}/100. Use the mock draft before the real draft starts.</div></button>}
        {waiverTargets[0] && <button className="action clickable" onClick={() => setTab("waivers")}><strong>Waiver radar: {nameOf(waiverTargets[0].player)}</strong><div className="muted">Trending add and currently unrostered in your league.</div></button>}
        {!lineupMoves.length && !injuries.length && !practiceMode && !waiverTargets.length && <div className="empty">Nothing urgent detected. Refresh closer to kickoff.</div>}
      </div></section>
...
function DraftView({ league, activeDraft, realPicks, practiceMode, practicePicks, players, recommendations, onPick, onUndo, onReset, onSimulateOthers }: any) {
  const top = recommendations[0];
  const mine = practicePicks.filter((p: PracticePick) => p.mine);
  const pickNumber = practicePicks.length + 1;
  return <div className="grid">
    <section className="card span12 draftToolbar">
      <div><div className="pill">{practiceMode ? "PRACTICE MODE" : "LIVE SLEEPER DRAFT"}</div><h2>{practiceMode ? `Overall mock pick #${pickNumber}` : `${realPicks.length} picks completed`}</h2><p className="muted">{practiceMode ? "Pick for yourself, then simulate the other managers. Recommendations recalculate as the board changes." : "Refresh periodically during the draft. Sleeper picks are removed automatically."}</p></div>
      {practiceMode && <div className="row"><button className="btn" onClick={onSimulateOthers}>Simulate other managers</button><button className="btn secondary" onClick={onUndo} disabled={!practicePicks.length}>Undo</button><button className="btn secondary" onClick={onReset} disabled={!practicePicks.length}>Reset</button></div>}
    </section>
...
function LineupView({ league, roster, players, moves, weeklyProjection, currentWeek }: any) {
  ...
  return <div className="grid">
    <section className="card span12"><div className="eyebrow">WEEK {currentWeek}</div><h2 className="sectionTitle">Start / sit assistant</h2><p className="muted">Sleeper is read-only, so this dashboard recommends changes; you make the final lineup change inside Sleeper.</p></section>
    {moves.length > 0 && <section className="card span12 recommendationPanel"><div className="eyebrow">RECOMMENDED CHANGES</div><div className="actions">{moves.map((move: any) => <div className="lineupMove" key={`${move.in.player_id}-${move.out.player_id}`}><div className="swap"><span className="benchBadge">BENCH</span><strong>{nameOf(move.out)}</strong><span className="arrow">→</span><span className="startBadge">START</span><strong>{nameOf(move.in)}</strong></div><div className="positive">+{move.delta} projected pts · {move.confidence}</div></div>)}</div></section>}
    {injuries.length > 0 && <section className="card span12 injuryPanel"><div className="eyebrow">INJURY WATCH</div><div className="list">{injuries.map((p: SleeperPlayer) => <div className="item" key={p.player_id}><span><strong>{nameOf(p)}</strong><span className="muted tiny">{p.position} · {p.team ?? "FA"} · {p.injury_body_part ?? "injury"}</span></span><span className="tag danger">{p.injury_status}</span></div>)}</div></section>}
    <section className="card span7"><div className="eyebrow">STARTERS</div><div className="list">{starters.map((id: string, index: number) => <PlayerLine key={id} player={players[id]} slot={league.roster_positions[index] ?? players[id]?.position} projection={weeklyProjection(id)} />)}</div></section>
...
function PlayerLine({ player, slot, projection }: { player?: SleeperPlayer; slot: string; projection?: number }) {
  return <div className="item"><span className="playerRow"><span className="slotBadge">{slot}</span><span><strong>{nameOf(player)}</strong><span className="muted tiny">{player?.position ?? "?"} · {player?.team ?? "FA"}{player?.injury_status ? ` · ${player.injury_status}` : ""}</span></span></span><span><strong>{projection != null ? projection.toFixed(1) : "—"}</strong><span className="muted tiny">proj.</span></span></div>;
}
...
function WaiverView({ targets, drops, roster, players, weeklyProjection, onPlayer }: any) {
  ...
  return <div className="grid">
    ...
    <section className="card span8"><div className="list">{targets.length ? targets.map((x: any, i: number) => {
      const samePosDrop = worstBench.map((id: string) => players[id]).find((p: SleeperPlayer | undefined) => p && playerPosition(p) === playerPosition(x.player));
      return <button className="item clickable" key={x.player.player_id} onClick={() => onPlayer(x.player.player_id)}><span><strong>#{i + 1} {nameOf(x.player)}</strong><span className="muted tiny">{x.player.position} · {x.player.team ?? "FA"} · {x.trend.count} trending adds{x.projection != null ? ` · ${x.projection.toFixed(1)} week proj.` : ""}{samePosDrop ? ` · possible drop: ${nameOf(samePosDrop)}` : ""}</span></span><span className="score">{x.score}</span></button>;
    }) : <div className="empty">No waiver candidates loaded yet.</div>}</div></section>
    <section className="card span4"><div className="eyebrow">TRENDING DOWN</div><h2 className="sectionTitle">Players people are dropping</h2><div className="list compact">{drops.slice(0, 15).map((d: any) => <div className="item" key={d.player_id}><span><strong>{nameOf(players[d.player_id])}</strong><span className="muted tiny">{players[d.player_id]?.position ?? "?"}</span></span><span>{d.count}</span></div>)}</div></section>
  </div>;
}
...
function PlayersView({ players, query, setQuery, rosteredIds, weeklyProjection, seasonProjection, onPlayer }: any) {
  const filtered = Object.values(players)
    .filter((p) => fantasyPositions.includes(playerPosition(p) ?? "") && p.status !== "Inactive" && nameOf(p).toLowerCase().includes(query.toLowerCase()))
    .sort((a, b) => (seasonProjection(b.player_id) ?? 0) - (seasonProjection(a.player_id) ?? 0))
    .slice(0, 150);
...
function LeagueView({ league, rosters, matchupData, transactions, players, myRoster, weeklyProjection, currentWeek }: any) {
  ...
  return <div className="grid">
    <section className="card span5"><div className="eyebrow">SETTINGS</div><h2 className="sectionTitle">League format</h2><div className="list compact"><div className="item"><span>Teams</span><strong>{league.total_rosters}</strong></div><div className="item"><span>Scoring</span><strong>{formatScoring(scoringKey(league.scoring_settings))}</strong></div><div className="item"><span>Roster</span><strong>{league.roster_positions.length} slots</strong></div><div className="item"><span>Status</span><strong>{league.status}</strong></div></div><h3>Roster slots</h3><div className="tagCloud">{league.roster_positions.map((p: string, i: number) => <span className="pill" key={`${p}-${i}`}>{p}</span>)}</div></section>
    <section className="card span7"><div className="eyebrow">SCORING</div><h2 className="sectionTitle">Rules that affect your points</h2><div className="scoringGrid">{Object.entries(league.scoring_settings).sort((a: any, b: any) => a[0].localeCompare(b[0])).map(([key, value]: any) => <div className="item" key={key}><span>{pretty(key)}</span><strong>{value}</strong></div>)}</div></section>
    <section className="card span7"><div className="eyebrow">WEEK {currentWeek} MATCHUPS</div><h2 className="sectionTitle">League scoreboard</h2><div className="list">{pairMatchups(matchupRows).map((pair, i) => <div className={`matchup ${pair.some((m) => m.roster_id === myRoster?.roster_id) ? "myMatchup" : ""}`} key={i}>{pair.map((m) => { const roster = rosters.find((r: SleeperRoster) => r.roster_id === m.roster_id); return <div className="matchTeam" key={m.roster_id}><strong>Roster {m.roster_id}{m.roster_id === myRoster?.roster_id ? " (YOU)" : ""}</strong><span>{m.points ?? 0} pts</span><span className="muted tiny">{sumProjected(roster?.starters ?? [], weeklyProjection)?.toFixed(1) ?? "—"} projected</span></div>; })}</div>)}</div>{!matchupRows.length && <div className="empty">Matchups will appear once Sleeper generates them.</div>}</section>
    <section className="card span5"><div className="eyebrow">RECENT ACTIVITY</div><h2 className="sectionTitle">Transactions</h2><div className="list compact">{transactions.slice(0, 20).map((t: SleeperTransaction) => <div className="item" key={t.transaction_id}><span><strong>{pretty(t.type)}</strong><span className="muted tiny">{transactionSummary(t, players)}</span></span><span className="pill">{t.status}</span></div>)}</div>{!transactions.length && <div className="empty">No transactions this week.</div>}</section>
  </div>;
}

function GuideView() {
  return <div className="grid guideGrid">
    <section className="card span12"><div className="eyebrow">ZERO-NFL-KNOWLEDGE GUIDE</div><h2>Only learn the things that change your fantasy decisions</h2><p className="muted">You do not need to understand formations, defensive schemes or most NFL rules to manage a fantasy team.</p></section>
    <GuideCard title="WR — Wide Receiver" body="Catches passes. Usually one of your most important fantasy positions. Targets = how often the quarterback throws toward them." />
    <GuideCard title="RB — Running Back" body="Runs the ball and often catches short passes. Valuable because reliable high-volume RBs are scarce." />
    <GuideCard title="QB — Quarterback" body="Touches the ball constantly and scores lots of raw fantasy points, but in normal one-QB leagues there are many usable options, so you often do not need to draft one early." />
    <GuideCard title="TE — Tight End" body="A hybrid receiver/blocker. Fantasy production is top-heavy: a few are great, many are similar." />
    <GuideCard title="FLEX" body="A lineup slot that can usually hold RB/WR/TE. This is why depth at RB and WR matters." />
    <GuideCard title="Bye week" body="That NFL team does not play that week. The player scores zero, so move them out of your starting lineup." />
    <GuideCard title="Waivers" body="The pool of players nobody in your league currently owns. Injuries and role changes can suddenly make an unknown player very valuable." />
    <GuideCard title="Questionable / Doubtful / Out" body="Injury designations. Out means do not start them. Questionable means check closer to kickoff." />
    <GuideCard title="Projection" body="Estimated fantasy points. Useful, not guaranteed. A 1-point difference is basically a coin flip; 5+ points is much more meaningful." />
  </div>;
}

function GuideCard({ title, body }: { title: string; body: string }) {
  return <section className="card span4"><h3>{title}</h3><p className="muted">{body}</p></section>;
}
...
function PlayerDrawer({ player, rostered, weekly, season, onClose }: { player: SleeperPlayer; rostered: boolean; weekly?: number; season?: number; onClose: () => void }) {
  return <div className="drawerBackdrop" onClick={onClose}><aside className="drawer" onClick={(e) => e.stopPropagation()}><button className="closeButton" onClick={onClose}>×</button><div className="eyebrow">PLAYER DETAILS</div><h2>{nameOf(player)}</h2><div className="muted">{player.position} · {player.team ?? "Free agent"} · Age {player.age ?? "?"}</div><div className="drawerStats"><div><span>Week projection</span><strong>{weekly?.toFixed(1) ?? "—"}</strong></div><div><span>Season projection</span><strong>{season?.toFixed(1) ?? "—"}</strong></div><div><span>League status</span><strong>{rostered ? "Rostered" : "Available"}</strong></div><div><span>Health</span><strong>{player.injury_status ?? "No flag"}</strong></div></div><h3>Plain English</h3><p className="muted">{plainEnglish(player, weekly)}</p>{player.injury_notes && <div className="action warn"><strong>Injury note</strong><div className="muted">{player.injury_notes}</div></div>}</aside></div>;
}

function eligibleForSlot(player: SleeperPlayer, slot: string) {
...
function sumProjected(ids: string[], getProjection: (id: string) => number | undefined) {
  const values = ids.map(getProjection).filter((x): x is number => typeof x === "number");
  if (!values.length) return undefined;
  return values.reduce((sum, value) => sum + value, 0);
}

function pairMatchups(rows: SleeperMatchup[]) {
  const groups = new Map<number, SleeperMatchup[]>();
  rows.forEach((m) => { if (m.matchup_id != null) groups.set(m.matchup_id, [...(groups.get(m.matchup_id) ?? []), m]); });
  return Array.from(groups.values());
}

function transactionSummary(t: SleeperTransaction, players: PlayerMap) {
  const adds = Object.keys(t.adds ?? {}).map((id) => nameOf(players[id]));
  const drops = Object.keys(t.drops ?? {}).map((id) => nameOf(players[id]));
  if (t.type === "trade") return `Between roster${t.roster_ids.length > 1 ? "s" : ""} ${t.roster_ids.join(" & ")}`;
  if (adds.length && drops.length) return `Added ${adds.join(", ")} · dropped ${drops.join(", ")}`;
  if (adds.length) return `Added ${adds.join(", ")}`;
  if (drops.length) return `Dropped ${drops.join(", ")}`;
  return `Roster ${t.roster_ids.join(", ")}`;
}

function plainEnglish(player: SleeperPlayer, weekly?: number) {
  const pos = playerPosition(player);
  if (player.injury_status === "Out") return "Do not start this player while they are ruled out.";
  if (player.injury_status) return "There is an injury flag. Re-check their status close to kickoff before relying on them.";
  if (weekly != null && weekly >= 18) return `A strong weekly projection for a ${pos ?? "player"}. Usually someone you want in the lineup unless your alternatives are elite.`;
  if (weekly != null && weekly >= 12) return `A usable weekly option. Compare them against your other ${pos ?? "same-position"} choices before kickoff.`;
  return `No major warning detected. Use projection, roster need and availability rather than name recognition.`;
}

function formatScoring(key: string) {
...
function tabLabel(tab: Tab) {
  const labels: Record<Tab, string> = { dashboard: "Dashboard", draft: "Draft", lineup: "Lineup", waivers: "Waivers", players: "Players", league: "League", guide: "NFL 101" };
  return labels[tab];
}

function nameOf(p?: SleeperPlayer) {
  return p?.full_name || `${p?.first_name ?? "Unknown"} ${p?.last_name ?? "player"}`.trim();
}

function pretty(s: string) {
  return s.replace(/_/g, " ").replace(/([A-Z])/g, " $1").replace(/^./, (c) => c.toUpperCase());
}